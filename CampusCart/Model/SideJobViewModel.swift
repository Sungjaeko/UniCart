...
let COLLECTION_SIDEJOB = "sidejobs"
let PAGE_LIMIT_SIDEJOB = 20

enum SideJobServiceError: Error {
    case mismatchedDocumentError
    case unexpectedError
}

class SideJobViewModel: ObservableObject {
    private let db = Firestore.firestore()
    @Published var error: Error?
    @Published var posts: [NoImageListing] = []

    
    func createPost(sidejobPost: NoImageListing) -> String {
        var ref: DocumentReference? = nil
        
        // addDocument is one of those “odd” methods.
        ref = db.collection(COLLECTION_SIDEJOB).addDocument(data: [
            "title": sidejobPost.title,
            "date": sidejobPost.date, // This gets converted into a Firestore Timestamp.
            "description": sidejobPost.description
        ]) { possibleError in
    ...
    func fetchPosts() async throws -> [NoImageListing] {
        let postQuery = db.collection(COLLECTION_SIDEJOB)
            .order(by: "date", descending: true)
            .limit(to: PAGE_LIMIT_SIDEJOB)
